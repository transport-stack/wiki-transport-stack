...
- **Manual Inputs:**
    - Data on revenue, ridership, or one-off operational changes may be collected through spreadsheets or forms manually
      filled by transit personnel.
...
| route_id | stop_sequence | stop_id | arrival_time | departure_time |
|----------|---------------|---------|--------------|----------------|
| 101      | 1             | 1001    | 08:00:00     | 08:05:00       |
| 102      | 2             | 1002    | 08:10:00     | 08:15:00       |

#### Vehicle Data Template:

| vehicle_id | fuel_type | depot         | agency |
|------------|-----------|---------------|--------|
| V001       | Diesel    | Central Depot | DMRC   |
| V002       | Electric  | West Depot    | DTC    |

---

## Data Cleaning and Processing

Once data is collected, cleaning and preprocessing are crucial to ensure that the datasets are accurate, complete, and
...
- **Location Data:** Ensure `stop_lat` and `stop_lon` values are within the service area and adhere to precision
  requirements (e.g., up to 4 decimal places).
- **Schedule Data:** Validate that `arrival_time` is earlier than `departure_time` for each stop.
- **Relationships:** Cross-check dependencies between datasets (e.g., `stop_id` in schedules matches entries in the
  stops list).
...
A robust utility for transforming raw data into structured formats ensures compatibility with downstream systems. Using
Python and libraries like Pandas, NumPy, or GeoPandas, you can automate and standardize data transformation.
